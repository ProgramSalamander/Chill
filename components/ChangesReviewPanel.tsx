...
import { IconFilePlus, IconTrash, IconEdit, IconSparkles, IconGitMerge } from './Icons';
import { AIPatch, StagedChange } from '../types';
...
const ChangeFileListItem: React.FC<{ 
  ...
  const getChangeConfig = () => {
    switch(type) {
      ...
      case 'patch': return { icon: <IconSparkles size={14}/>, color: 'text-vibe-glow' };
    }
  }
...
const ChangesReviewPanel: React.FC = () => {
    const { stagedChanges, patches, applyAllChanges, rejectAllChanges } = useAgentStore();
        ...
        <div className="flex flex-col h-full bg-vibe-900/50">
            <div className="p-4 border-b border-white/5 shrink-0 bg-white/[0.02]">
                <div className="flex items-center gap-2 mb-3">
                    ...
                    <div className="text-xs font-bold text-slate-200 uppercase tracking-wider">
                        AI Proposals ({stagedChanges.length + patches.length})
                    </div>
                </div>
                <div className="flex gap-2">
                    <button 
                       onClick={() => rejectAllChanges()}
                       disabled={(stagedChanges.length + patches.length) === 0}
            ...
            <div className="flex-1 overflow-y-auto p-2 custom-scrollbar space-y-4">
                ...
                {stagedChanges.length > 0 && (
                    <div>
                        <div className="px-2 mb-1 text-[9px] font-black text-slate-500 uppercase tracking-widest">File Operations</div>
                        {stagedChanges.map(change => (
                            <ChangeFileListItem 
                                key={change.id} 
                                label={change.path} 
                                type={change.type} 
                                onClick={() => handleSelectPath(change.path)} 
                            />
                        ))}
                    </div>
                )}

                {stagedChanges.length === 0 && patches.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-12 text-center px-4">
                        ...
                        <p className="text-xs text-slate-500 italic">No pending changes</p>
                    </div>
                )}
            </div>
