...
function getFilePath(file, allFilesMap) {
  let path = file.name;
  let current = file;
  let depth = 0;
...
function chunkFile(file, allFilesMap) {
    const CHUNK_SIZE = 20; 
...
self.onmessage = async (e) => {
    ...
    try {
        if (type === 'updateIndex') {
            ...
            const filesToIndex = files.filter((f) => f.type === 'file' && f.content.length > 0 && f.content.length < 100000);
